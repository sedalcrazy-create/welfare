...
@section('title', 'افزودن مرکز رفاهی')

@section('content')
<!-- Page Header -->
<div class="page-header">
    <h1><i class="bi bi-plus-circle"></i> افزودن مرکز رفاهی جدید</h1>
        ...
        <form action="{{ route('centers.store') }}" method="POST">
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                           value="{{ old('name') }}" placeholder="مثال: زائرسرای مشهد مقدس">
                    @error('name')
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="city" class="form-control @error('city') is-invalid @enderror"
                           value="{{ old('city') }}" placeholder="مثال: مشهد">
                    @error('city')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row mb-4">
                <div class="col-md-4">
                    ...
                    <select name="type" class="form-control @error('type') is-invalid @enderror">
                        ...
                        <option value="religious" {{ old('type') === 'religious' ? 'selected' : '' }}>زیارتی</option>
                        <option value="beach" {{ old('type') === 'beach' ? 'selected' : '' }}>ساحلی (تفریحی)</option>
                        <option value="mountain" {{ old('type') === 'mountain' ? 'selected' : '' }}>کوهستانی (تفریحی)</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="stay_duration" class="form-control @error('stay_duration') is-invalid @enderror"
                           value="{{ old('stay_duration', 3) }}" min="1" max="30">
                    @error('stay_duration')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4">
                    ...
                    <input type="text" name="phone" class="form-control @error('phone') is-invalid @enderror"
                           value="{{ old('phone') }}" placeholder="مثال: 051-12345678">
                    @error('phone')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6">
                    ...
                    <input type="time" name="check_out_time" class="form-control @error('check_out_time') is-invalid @enderror"
                           value="{{ old('check_out_time', '09:00') }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="address" class="form-control @error('address') is-invalid @enderror"
                          rows="2" placeholder="آدرس کامل مرکز">{{ old('address') }}</textarea>
                @error('address')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" class="form-control @error('description') is-invalid @enderror"
                          rows="3" placeholder="توضیحات تکمیلی درباره مرکز">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
